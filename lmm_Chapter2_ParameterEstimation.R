...
hsb <- hsb %>% 
  mutate(sid = as_factor(paste0('S', 1:nrow(.))),
         id  = as_factor(id)) %>% 
  select(sid, id, female, ses, mathach, size, catholic) %>%
  as_tibble

?hsb # show description of data set

coef(lm(hsb$mathach~hsb$ses))
hsb %>% filter(id == id[1]) %>%
...
mean(hsb$mathach[hsb$id == hsb$id[1]])  

# centering
hsb = hsb %>%
  group_by(id) %>%
  mutate(ses2 = ses - mean(ses)) %>%
  ungroup
